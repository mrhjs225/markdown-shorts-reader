...
function extractText(node2) {
  ...
  if (node2.type === "image" && typeof node2.alt === "string") {
    return node2.alt;
  }
  if (!Array.isArray(node2.children)) {
    return "";
  }
  return node2.children.map(extractText).join("");
}

// src/markdown/splitLongCard.ts
...
function splitTextCard(card, targetChars) {
...
function splitIntoSentences(content3) {
  const normalized = content3.replace(/\s+/g, " ").trim();
  if (!normalized) {
    return [];
  }
...
function chunkByLength(content3, targetChars) {
  const chunks = [];
  let cursor = 0;
  while (cursor < content3.length) {
    chunks.push(content3.slice(cursor, cursor + targetChars).trim());
    cursor += targetChars;
  }
  return chunks.filter(Boolean);
}
function createDerivedCard(original, content3, index2) {
  return {
    ...original,
    id: `${original.id}-${index2 + 1}`,
    content: content3,
    type: original.type === "paragraph" ? "paragraph" : "mixed",
    meta: {
      ...original.meta,
      charCount: content3.length,
      wordCount: countWords(content3),
      isLong: false
    }
  };
}
function countWords(content3) {
  return content3.trim().split(/\s+/u).filter(Boolean).length;
}

// src/markdown/chunkMarkdown.ts
var DEFAULT_MIN_CARD_CHARS = 120;
var DEFAULT_MAX_CARD_CHARS = 700;
var DEFAULT_LONG_CARD_CHARS = 1e3;
function chunkMarkdown(markdown, options = {}) {
  ...
  for (const node2 of tree.children ?? []) {
    const card = createCardFromNode(node2, sectionPath, sourceLines);
    if (card) {
      rawCards.push(card);
    }
  }
...
function createCardFromNode(node2, sectionPath, sourceLines) {
  if (node2.type === "heading") {
    return createHeadingCard(node2, sectionPath);
  }
  if (node2.type === "paragraph") {
    if (isImageParagraph(node2)) {
      const imageNode = node2.children?.[0];
      const content4 = imageNode?.alt?.trim() || imageNode?.url?.trim() || "Image";
      return createTextCard(node2, "image", content4, sectionPath, {
        imageUrl: imageNode?.url
      });
    }
    const content3 = extractText(node2).trim();
    return createTextCard(node2, "paragraph", content3, sectionPath);
  }
  if (node2.type === "code") {
    const content3 = node2.value ?? "";
    return createTextCard(node2, "code", content3, sectionPath, {
      language: node2.lang,
      isMermaid: node2.lang?.toLowerCase() === "mermaid",
      isLong: content3.length > 1200
    });
  }
  if (node2.type === "list") {
    return createTextCard(node2, "list", getSourceForNode(node2, sourceLines), sectionPath);
  }
  if (node2.type === "blockquote") {
    return createTextCard(node2, "blockquote", getSourceForNode(node2, sourceLines), sectionPath);
  }
  if (node2.type === "thematicBreak") {
    return createTextCard(node2, "thematicBreak", "---", sectionPath);
  }
  if (node2.type === "table") {
    return createTextCard(node2, "table", getSourceForNode(node2, sourceLines), sectionPath);
  }
  if (node2.type === "html") {
    return createTextCard(node2, "mixed", node2.value?.trim() ?? "", sectionPath);
  }
  return void 0;
}
function createHeadingCard(node2, sectionPath) {
  const title = extractText(node2).trim();
  const level = Math.max(1, Math.min(node2.depth ?? 1, 6));
  sectionPath.length = level - 1;
  sectionPath[level - 1] = title;
  const card = {
    id: createCardId("heading", title, getStartLine(node2), getEndLine(node2)),
    type: "heading",
    title,
    content: title,
    level,
    startLine: getStartLine(node2),
    endLine: getEndLine(node2),
    sectionPath: [...sectionPath],
    meta: {
      charCount: title.length,
      wordCount: countWords2(title)
    }
  };
  return card;
}
function createTextCard(node2, type, content3, sectionPath, meta = {}) {
  const startLine = getStartLine(node2);
  const endLine = getEndLine(node2);
  return {
    id: createCardId(type, content3, startLine, endLine),
    type,
    ...
    meta: {
      ...meta,
      charCount: content3.length,
      wordCount: countWords2(content3)
    }
  };
}
function mergeSmallCards(cards, minCardChars, maxCardChars) {
  const result = [];
  let buffer;
...
function shouldTryMerge(card, minCardChars) {
...
function canMerge(previous3, next, maxChars) {
  ...
  if (previous3.type === "blockquote" || next.type === "blockquote") {
    return previous3.type === next.type && sameSection(previous3, next);
  }
  if (!sameSection(previous3, next)) {
    return false;
  }
  const combinedLength = previous3.content.length + next.content.length + 2;
  return combinedLength <= maxChars;
}
function sameSection(previous3, next) {
  return previous3.sectionPath.join("/") === next.sectionPath.join("/");
}
function mergeCards(previous3, next) {
  const content3 = `${previous3.content}

${next.content}`;
  return {
    ...previous3,
    id: `${previous3.id}_${next.id}`,
    type: previous3.type === next.type ? previous3.type : "mixed",
    content: content3,
...
function getCurrentSectionTitle(sectionPath) {
  return sectionPath.at(-1);
}
function getStartLine(node2) {
  return Math.max((node2.position?.start?.line ?? 1) - 1, 0);
}
function getEndLine(node2) {
  return Math.max((node2.position?.end?.line ?? 1) - 1, 0);
}
function isImageParagraph(node2) {
  return node2.children?.length === 1 && node2.children[0]?.type === "image";
}
function getSourceForNode(node2, sourceLines) {
  const startLine = getStartLine(node2);
  const endLine = getEndLine(node2);
  return sourceLines.slice(startLine, endLine + 1).join("\n").trim();
}
function countWords2(content3) {
  return content3.trim().split(/\s+/u).filter(Boolean).length;
}
function createCardId(type, content3, startLine, endLine) {
  return `${type}-${startLine}-${endLine}-${hashString(content3)}`;
}
function createDocumentReviewCard(cards) {
  const issues = [
  ...
  return {
    id: createCardId("review", content3, 0, 0),
    type: "review",
...
function findHeadingLevelIssues(cards) {
  const issues = [];
  let previousLevel = 0;
  for (const card of cards) {
...
function findEmptySectionIssues(cards) {
  return cards.filter((card) => card.type === "heading").filter((heading2) => !sectionHasBody(cards, heading2)).map((heading2) => ({
    line: heading2.startLine,
    message: `Section "${heading2.content}" has no body content.`
  }));
}
function findOversizedSectionIssues(cards) {
  const maxSectionChars = 5e3;
  return cards.filter((card) => card.type === "heading").map((heading2) => ({
    heading: heading2,
    charCount: getSectionCards(cards, heading2).reduce(
      (total, card) => total + (card.meta?.charCount ?? card.content.length),
      0
    )
  })).filter(({ charCount }) => charCount > maxSectionChars).map(({ heading: heading2, charCount }) => ({
    line: heading2.startLine,
    message: `Section "${heading2.content}" is large (${charCount} characters). Consider splitting it.`
  }));
}
function sectionHasBody(cards, heading2) {
  return getSectionCards(cards, heading2).some(
    (card) => card.type !== "heading" && card.type !== "thematicBreak"
  );
}
function getSectionCards(cards, heading2) {
  const startIndex = cards.indexOf(heading2);
  if (startIndex < 0) {
    return [];
  }
  const result = [];
  for (let index2 = startIndex + 1; index2 < cards.length; index2 += 1) {
    const card = cards[index2];
    if (card.type === "heading" && (card.level ?? 1) <= (heading2.level ?? 1)) {
      break;
    }
    result.push(card);
  }
...
function hashString(value) {
  ...
  for (let index2 = 0; index2 < value.length; index2 += 1) {
    hash = hash * 33 ^ value.charCodeAt(index2);
  }
  return (hash >>> 0).toString(36);
}

// src/state/readingState.ts
var STORAGE_KEY = "mdShorts.readingState";
function getReadingState(context, documentUri) {
  const states = context.workspaceState.get(STORAGE_KEY) ?? {};
  return states[documentUri];
}
async function saveReadingState(context, state) {
  const states = context.workspaceState.get(STORAGE_KEY) ?? {};
  ...
  <script nonce="${nonce}">
    ...
    function getCardContent(card) {
      if (isCodeCollapsed(card)) {
        ...
        return "Code block collapsed \xB7 " + String(lineCount) + " lines";
      }
...
async function openReader(context) {
  ...
  const markdown = document3.getText();
  if (markdown.length > 1e6) {
  ...
  const documentUri = document3.uri.toString();
  const rememberPosition = config.get("rememberPosition", true);
  const previousState = rememberPosition ? getReadingState(context, documentUri) : void 0;
  const initialIndex = getInitialIndex(
    cards,
    previousState?.currentCardId,
    previousState?.currentIndex
  );
  const openBeside = config.get("openBeside", true);
  let currentCards = cards;
  const panel = vscode.window.createWebviewPanel(
    "mdShorts.reader",
    `MD Shorts: ${getFileName(document3.uri)}`,
    openBeside ? vscode.ViewColumn.Beside : vscode.ViewColumn.Active,
    {
      enableScripts: true,
      retainContextWhenHidden: true,
      localResourceRoots: getLocalResourceRoots(document3.uri)
    }
  );
  currentCards = resolveWebviewImageUris(cards, panel.webview, document3.uri);
  ...
  panel.webview.onDidReceiveMessage(async (message) => {
    if (message.type === "revealSource") {
      await revealSource(document3.uri, currentCards[message.index], editor.viewColumn);
      return;
    }
    if (message.type === "savePosition" && rememberPosition) {
      await saveReadingState(context, {
        ...
        bookmarkedCardIds: Array.isArray(message.bookmarkedCardIds) ? message.bookmarkedCardIds.filter((cardId) => typeof cardId === "string") : previousState?.bookmarkedCardIds,
        readCardIds: Array.isArray(message.readCardIds) ? message.readCardIds.filter((cardId) => typeof cardId === "string") : previousState?.readCardIds,
        collapsedCodeCardIds: Array.isArray(message.collapsedCodeCardIds) ? message.collapsedCodeCardIds.filter((cardId) => typeof cardId === "string") : previousState?.collapsedCodeCardIds,
        typeFilter: isKnownTypeFilter(message.typeFilter) ? message.typeFilter : previousState?.typeFilter,
...
function createCards(markdown, config) {
  return chunkMarkdown(markdown, {
    minCardChars: config.get("minCardChars", 120),
    maxCardChars: config.get("maxCardChars", 700)
  });
}
async function revealSource(uri, card, preferredColumn) {
  if (!card) {
    return;
  }
  const document3 = await vscode.workspace.openTextDocument(uri);
...
function getInitialIndex(cards, currentCardId, currentIndex) {
  if (currentCardId) {
...
function isKnownTypeFilter(value) {
  return value === "all" || value === "heading" || value === "paragraph" || value === "code" || value === "list" || value === "blockquote" || value === "image" || value === "table" || value === "thematicBreak" || value === "review" || value === "mixed";
}
function getLocalResourceRoots(documentUri) {
  if (documentUri.scheme !== "file") {
    return [];
  }
  return [vscode.Uri.joinPath(documentUri, "..")];
}
function resolveWebviewImageUris(cards, webview, documentUri) {
...
function getImageWebviewUri(imageUrl, webview, documentUri) {
  ...
  const imageUri = path2.isAbsolute(imagePath) ? vscode.Uri.file(imagePath) : vscode.Uri.joinPath(documentUri, "..", imagePath);
